...
public class ArbolAVL {
    ...
    public ListaEnlazadaDesordenada<Producto> buscar(Producto p) {
        ListaEnlazadaDesordenada<Producto> resultado = new ListaEnlazadaDesordenada<>();
    ...
    private void buscarRecursivo(NodoAVL nodo, String nombre, ListaEnlazadaDesordenada<Producto> resultado) {
        if (nodo == null) {
            return;
        }
        ...
        if (cmp < 0) {
            ...
            buscarRecursivo(nodo.getIzquierdo(), nombre, resultado);
            resultado.insertar(nodo.getProducto());
            buscarRecursivo(nodo.getDerecho(), nombre, resultado);
        }
    }
    ...
    private void inorderRecursivo(NodoAVL nodo, StringBuilder sb) {
        if (nodo == null) {
            return;
        }

        inorderRecursivo(nodo.getIzquierdo(), sb);

        Producto p = nodo.getProducto();
        sb.append(p.getNombre())
                .append(" (")
                .append(p.getCodigoBarras())
                .append(")\n");

        inorderRecursivo(nodo.getDerecho(), sb);
    }
